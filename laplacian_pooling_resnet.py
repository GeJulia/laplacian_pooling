...
from laplace_pooling import Laplacian_Pooling
...
# replacement of Pooling Layers inspired by https://arxiv.org/pdf/1904.11486.pdf
# the corresponding GitHub can be found https://github.com/adobe/antialiased-cnns
class BasicBlock(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1, 
                 base_width=64, dilation=1, norm_layer=None, filter_size=1, sigma=1):
        ...
        if(stride==1):
            ...
            self.conv2 = nn.Sequential(Laplacian_Pooling(planes, filt_size=5, stride=stride, sigma=sigma),
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1, 
        ...
        if(stride==1):
            ...
            self.conv3 = nn.Sequential(Laplacian_Pooling(width, filt_size=5, stride=stride, sigma=sigma),
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
                 ...
                 replace_stride_with_dilation=None, sigma=1):
        ...
        if(pool_only):
            ...
            self.maxpool = nn.Sequential(*[nn.MaxPool2d(kernel_size=2, stride=1), 
                Laplacian_Pooling(self.inplanes, filt_size=5, stride=2, sigma=sigma)])
        else:
            self.conv1 = nn.Conv2d(3, self.inplanes, kernel_size=7, stride=1, padding=3, bias=False)
            self.bn1 = norm_layer(self.inplanes)
            self.relu = nn.ReLU(inplace=True)
            self.maxpool = nn.Sequential(*[Laplacian_Pooling(self.inplanes, filt_size=filter_size, stride=2, sigma=sigma), 
    ...
    def _make_layer(self, block, planes, blocks, stride=1, filter_size=1, dilate=False):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            conv_inplanes = self.inplanes
            if(stride !=1):
                downsample = [Laplacian_Pooling(filt_size=5, stride=stride, channels=self.inplanes, sigma=self.sigma)]
            else:
                downsample = []
            downsample += [conv1x1(conv_inplanes, planes * block.expansion, 1),
...
def resnet152(pretrained=False, filter_size=5, pool_only=True, _force_nonfinetuned=False, sigma=1, **kwargs):
    """Constructs a ResNet-152 model."""
    model = ResNet(Bottleneck, [3, 8, 36, 3], filter_size=filter_size, pool_only=pool_only, sigma=sigma, **kwargs)
    return model
